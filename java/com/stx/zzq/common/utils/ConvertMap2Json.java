...
public class ConvertMap2Json {
	 ...
	 * @return
	 */
	public static String buildJsonBody(Map<String, Object> body, int tabCount,
			boolean addComma) {
		...
		for (String key : keySet) {
			count++;
			sbJsonBody.append(buildJsonField(key, body.get(key), tabCount + 1,
					count != size));
		}
	 ...
	 * @return
	 */
	private static String buildJsonField(String key, Object value,
			int tabCount, boolean addComma) {
	 ...
	 * @return
	 */
	private static String buildJsonValue(Object value, int tabCount,
			boolean addComma) {
		...
		if (value instanceof String) {
			...
			sbJsonValue.append(QUOTE)
					.append(formatDate((java.util.Date) value)).append(QUOTE);
		} else if (value.getClass().isArray()
				|| value instanceof java.util.Collection) {
			sbJsonValue.append(buildJsonArray(value, tabCount, addComma));
		} else if (value instanceof java.util.Map) {
			sbJsonValue.append(buildJsonBody((java.util.Map) value, tabCount,
					addComma));
		}
	...
	private static String buildJsonArray(Object value, int tabCount,
		...
		for (Object obj : objArray) {
			...
			sbJsonArray.append(buildJsonValue(obj, tabCount + 1,
					++count != size));
		}
